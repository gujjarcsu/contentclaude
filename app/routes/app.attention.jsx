    ...
    <Card>
      <BlockStack gap="200">
        <Text as="h2" variant="headingSm">
          {t("Google's AI features — the one check we ask you to make")}
        </Text>
        <Text as="p" variant="bodySm">
          {t("Google lets a site owner exclude a site from AI Overviews, AI Mode and AI in Discover: in Search Console, under Settings, the switch is called Search generative AI. No app can read it — Google offers no API for it — so this is the one check we ask you to make yourself. It takes thirty seconds, and an agency or a previous developer may have set it without saying so.")}
        </Text>
        {gsc?.answer ? (
          <BlockStack gap="200">
            ...
            {gsc.excluded && (
              <Text as="p" variant="bodySm">
                {t("While the switch is on, your pages do not appear in AI Overviews or AI Mode. Regular Google results are unaffected. If that was not your decision, switch it off in Search Console; it can take days to take effect.")}
              </Text>
            )}
        ...
        <Text as="p" variant="bodySm" tone="subdued">
          {t("Method: your answer, kept with its date and asked again after {GSC_RECHECK_DAYS} days. Nothing here is read from Google — there is no API that would let us.", { GSC_RECHECK_DAYS })}
        </Text>
        <Text as="p" variant="bodySm" tone="subdued">
          {t("While you are in Search Console:")} <Link url="/app/ai-reports">{t("the two AI reports you read yourself")}</Link> {t("— Google's generative-AI report and Bing's AI Performance, neither of which has an API.")}
        </Text>
      </BlockStack>
    </Card>
  );
...
export default function AttentionPage() {
  ...
  return (
    <Page
      ...
      subtitle={summary.available ? lines[0] ?? t("Nothing needs you right now.") : t("We have not been able to read your catalogue yet.")}
    ...
    >
      <BlockStack gap="400">
        {summary.partial && (
          <Card>
            <Text as="p" variant="bodySm" tone="subdued">
              {t("Your catalogue is larger than one check covers, so this page is from the products we reached. The daily check continues where it left off.")}
            </Text>
          </Card>
        )}
        ...
        <Card>
          <BlockStack gap="200">
            ...
            {!crawler.available ? (
              <Text as="p" variant="bodySm" tone="subdued">
                {t("Checked every night against your storefront. The first result appears after tonight's check.")}
              </Text>
            ) : crawler.passwordProtected ? (
              <Text as="p" variant="bodySm">
                {t("Your storefront is password-protected, so every crawler — and every shopper — gets the password page instead of your products. That is expected before launch. Remove the password when you open and we check again the same night.")}
              </Text>
              ...
              crawler.blocked.map((agent) => (
                <BlockStack key={agent} gap="100">
                  <InlineStack gap="200" blockAlign="center">
                    <Badge tone="critical">{t("{agent} blocked", { agent })}</Badge>
        ...
        {nothing && (
          <Card>
            <EmptyState heading={t("Nothing needs attention")} image="">
              <p>
                {t("We check your catalogue every day for descriptions that collapse, alt text that disappears, changed URLs, missing product types, new products with nothing written yet, and anything an AI shopping surface asks for that a product lacks. When something changes, it appears here the same day.")}
              </p>
            </EmptyState>
          </Card>
        )}
                ...
                <BlockStack gap="200">
                  ...
                  {r.findings.map((f) => (
                    <BlockStack key={f.field} gap="100">
                      <InlineStack gap="200" blockAlign="center">
                        <Badge tone={TONE[f.grade] ?? "info"}>{t("Indexing · {field}", { field: f.field })}</Badge>
                        <Text as="span" variant="bodySm" tone="subdued">
                          {f.grade === "blocking" ? t("cannot be indexed") : t("indexed, but worse")}
                        </Text>
                      </InlineStack>
                      <Text as="p" variant="bodySm">
                        {st(f.note)}
        ...
        {gaps.length > 0 && (
          <BlockStack gap="200">
            <InlineStack align="space-between" blockAlign="center" wrap>
              <Text as="h2" variant="headingMd">
                {t("Missing what a surface asks for")}
              </Text>
        ...
        {rows.length > 0 && (
          <BlockStack gap="200">
            <Text as="h2" variant="headingMd">
              {t("Changed since we last looked")}
            </Text>
            {rows.map((r) => (
              <Card key={r.productId}>
                <BlockStack gap="200">
        ...
        <Text as="p" variant="bodySm" tone="subdued">
          {t("Method: a daily read of every product in your catalogue — title, description, vendor, product type, featured image and its alt text, URL handle, the barcodes of up to {VARIANT_BARCODE_SAMPLE}{v} variants when the first has none, and option names. Each product is compared with the previous day, and measured against the fields the OpenAI product feed and Google Search ask for. Blocking means a surface cannot list the product without it; degrading means listed, but worse; Shopify's own fields are never called either. Drafts are not graded. Indexing is read from your storefront the way a crawler reads it: your sitemap for every product, and a sample of product pages each night for robots directives, the declared canonical address and redirect hops — not from Search Console, which we cannot see. Nothing here is a ranking claim.", { VARIANT_BARCODE_SAMPLE, v: " " })}
        </Text>
      </BlockStack>
    </Page>
  );
