...
| **1 Oct 2026** | Extensions on API version **2025-07 or earlier cannot be updated**. | **CLOSED 2026-09-14 — it does not bind us (P0.1).** A theme app extension has **no `api_version` to declare**: its config takes `name`, `type` and `handle` only, and it is versioned as part of the APP version. Proof from our own build artefact: `.shopify/deploy-bundle/manifest.json` lists `navaal-geo-schema` as `theme_app_extension` with **no `api_version` field**, while six sibling modules in the same manifest carry `api_version: 2026-04`. This deadline is the **React → Polaris** cutover and binds the `ui_extension` family; we ship one extension and it is **pure Liquid**. `scripts/check-api-versions.mjs` now fails the build if a `ui_extension` is ever added below 2025-10. |
| **16 Oct 2026** | Admin API **2025-10 becomes inaccessible**. | **Identified and safe.** We are on **2026-04** — `ApiVersion.April26` in `app/shopify.server.js` for the Admin client, and `[webhooks] api_version` in `shopify.app.toml:14` — which sunsets **2027-04-01**, 199 days out as of 2026-09-14. **This is now enforced, not remembered:** `scripts/check-api-versions.mjs` runs in CI before `npm ci` and fails the build when any pin comes within 90 days of sunset. *(Correction: this row previously implied a single pin at `shopify.app.toml:14`. That line is the **webhooks** version; the Admin client's pin is in `app/shopify.server.js`. The check watches both and fails if they disagree.)* |
...
| Ongoing | **App Bridge `app-bridge.js` script tag before any other script, in the `<head>` of every document.** INP is only collected for apps using it. | **MEASURED FROM THE RENDERED DOCUMENT 2026-09-14 (P0.4), and this row was wrong.** It cited `app/utils/embedded.server.js:181`, which is the **/reembed recovery page** — one bounce page, not "every document". On a real authenticated `/app` document the tag was emitted by `<AppProvider embedded>` **inside the route tree, in the BODY**; React is 18.3.1 and does not hoist scripts (a React 19 feature), so the `<head>` contained **no scripts at all**. It was already the FIRST script overall (order 0) and `window.shopify` was true, so half the rule was met. Now declared in `root.jsx`'s head with the `shopify-api-key` meta. Re-read it with `node tools/proof/appbridge-head.mjs`. |
