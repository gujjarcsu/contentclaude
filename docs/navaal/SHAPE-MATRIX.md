...
| `navaal-shape-fr` | NON_ENGLISH (French product data) | `tools/proof/fixtures/shapes/fr.csv` | **RUN 2026-09-15 — half of this is false.** The publish keeps accents intact (`Mitigeur de cuisine 5 en laiton massif, finition chromée. Garantie 25 ans. Livraison sous 2 jours ouvrés. Seulement $24.00.`), but **the draft is written in English**: page title `Mitigeur de Cuisine 5 – Solid Brass Chrome Tap | Acme`, description in English prose, `Content quality: 90/100`. One draft approved and published to prove it. **CORRECTION:** the store's own `Content Language` is **English**, the default — `French` is in the dropdown. The writer obeyed its setting; the defect is that the first run set English on a store whose French it had already extracted into `keyDifferentiators` (`garantie 25 ans. livraison sous 2 jours ouvrés.`), and that `sampleContent` was seeded with `Your Privacy Choices: As described in our Privacy Policy, we`. |
...
**The import modal has a second lever, and the default destroys two of these shapes.** `Import products by CSV` carries a checkbox, verbatim **`Publish new products to all sales channels.`**, and it is **checked by default**. It is separate from the CSV's own `Published` column. **Uncheck it on every import here.** Read on screen 2026-09-15 with the box unchecked: `Published FALSE` (`alldraft.csv`, `b2b.csv`) → **Channels 0**; `Published TRUE` (`variants.csv`, `fr.csv`, `cap.csv`) → **Channels 1** and the product's Publishing card reads `Online Store`. So the column governs — but only if the box is cleared. Left at its default, the drafts and B2B stores come out published to every channel and neither shape exists any more. Shopify's preview banner also says *"will publish only to the Online Store"* with the box unchecked, on a file where nothing was published; believe the Channels column on Products, not the banner.
