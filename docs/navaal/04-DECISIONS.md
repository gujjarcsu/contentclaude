...
| **A date is formatted where it is shown, in the screen's locale; a loader ships an instant.** The usage card said "September" on a French screen because `planFit.js` formatted the quota month with `en-GB` in the loader. The loader now ships `monthAt`/`resetAt` as ISO and the screen calls `t.date` with the UTC format the quota key uses; Home's "since 3 September" and the Plans card's month and renewal date the same way. A guard forbids any fixed-English `toLocale*String` on a merchant path. (Phase 12 D3, `36d95ee`.) | ✔ |
| **Each language is held to ITS listing's register, never to another language's.** German and French address the merchant formally (Sie, vous), Spanish and Italian informally (tú, tu), Brazilian Portuguese with você, Japanese politely; the credit unit is the listing's word (Credits, crédits, créditos, crediti, créditos, クレジット); Free/Starter/Growth/Professional and Navaal are never translated. `tests/utils/i18nLocales.test.js` refuses the other register per locale with Unicode-aware boundaries, because JavaScript's `\b` is ASCII and "prêtes" ends in a "tes" it thinks is a word. (Phase 12 D2–D6.) | ✔ |
