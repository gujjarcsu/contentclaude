...
- **COWORK / OWNER — `navaal.ai/blog/feed.xml` was 8 posts behind the index before today (20 vs 28).** CW added the W1 item and corrected the weekday (`Sun` → `Mon`, 14 Sep 2026 is a Monday). The feed needs regenerating from the index in `navaal-platform`; a build step, not a hand edit. Routed to CC as low priority.
