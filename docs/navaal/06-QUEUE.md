...
- **COWORK VERIFIED CC AND CW FROM OUTSIDE, 2026-09-16. Both reports hold; one defect neither check was scoped to find, now fixed.** CC: production serves `7169eb1`, `startedAt` 12:44:26Z, and `?deep=1` now carries the new block — `scheduler.jobs` with `{ok, at}` per job, `failing: []`, `worstConsecutiveTicks: 0`, all four jobs `ok: true` with the timestamp advancing. Read again ten hours after the deploy, still `failing: []`, which is a far stronger window than the 319 s CC reported and the only part of its evidence Cowork could add to. CW: `6be476e` present; `OUTREACH-DRAFTS.md` holds exactly 30 drafts; a mechanical sweep of **the fenced message bodies alone** (not the prose around them, where the words legitimately appear inside each draft's own check) returns **0** for `barcode`, `guaranteed`, `71.9`, `409 stores`, `24/7`, `instantly`, `dedicated account manager`, `#1`, `SLA`, `best`, `free trial`; every stated word count is accurate and inside its cap, 30 of 30. Two counts re-run independently with `recheck.mjs`: `zilla-meals.com` **85 of 123** and `momarsh.com` **76 of 76 no product type, 27 of 76 short**, matching CW's table exactly.
- **THE DEFECT: a self-check scoped to DOCTRINE will not catch a mail-merge TELL.** 14 of the 30 drafts read *"a gap in how the supplements gets found"* — plural noun, singular verb — from a template whose slot was written for a singular category: `decals`, `cakes`, `meals`, `plants`, `quilt kits`, `graphic tees`, `calibration tools`, `farm goods`, `patches`, `cases`, `books`, `dresses`, `Pomsky listings`, `supplements`. Every one of CW's thirty checks passed, correctly — they ask "is this claim sourced?" and the answer was yes every time. None asks "does a founder reading this see a merge field?" **A broken verb in a personalised sentence is the single loudest signal that a human did not write the message, which is precisely what OUTREACH-PACK exists to prevent — so the doctrine check passed while the message failed at the thing doctrine is for.** Fixed by Cowork: plural subjects now take `get found`, 14 changed, 0 remaining, the singular ones untouched. **Rule added: a draft check needs a readability pass beside the sourcing pass, and the readability pass is read aloud, not grepped.**
- **OWNER — three slots must be filled by hand before ANY of the thirty goes out.** `{Owner's name}` ×28, `{owner email}` ×6, and `{their category}` ×1 in draft 30 (`shop.faithchurch.com`), which CW deliberately left blank rather than guess because the store gives no sourceable category from outside. A merge field that ships is worse than no message at all.

- **DONE — THE WEEKLY REPORT HAD NEVER RUN, AND THE DEFECT IS THE BUILD SHAPE RATHER THAN ONE FILE (CC, Phase 16, `982857b`).** `fly.toml` starts the worker as `node worker.js`, and `worker.js` imports `./app/utils/startup.server.js` directly — so **the worker loads the SOURCE tree with Node's own ESM loader while the web process runs the Vite bundle.** The path in the production error said so and nobody read it that way: `file:///app/app/i18n/locales/de.json` is *source*, not build. Node requires `with { type: "json" }`; Vite rewrites bare JSON imports at build time. `app/i18n/catalogues.server.js` was written for one of the two loaders. **The other three scheduled jobs did not throw for exactly one reason: none of them imports the i18n catalogues.** `weeklyReport.server.js` does, at line 25, because it writes merchant email in the merchant's language. Nothing about the shape was sound; only the reach differed — so this WILL come back through any module the worker reaches that imports JSON, and there is now a test that says so.
  - **The fix and what it cost.** Every JSON import in `catalogues.server.js` and `chunks.js` carries the attribute. **The client build is unchanged, proved by clean builds on both sides rather than by reasoning:** the same twelve locale chunks with the same content hashes and byte sizes (`de-CU5eynwi.js` 190,674 B and eleven more, identical either way), the same 92 files / 889.4 KB total, and no catalogue in the shared bundle. **Stale assets make this comparison lie** — `build/client/assets/` is not cleaned between builds, so the first "after" read showed twelve extra chunks that were simply the previous build's leftovers. Delete `build/` on both sides or the numbers are fiction.
  - **ESLint was the only real obstacle, and it is not Vite's fault.** espree 9.6 (ESLint 8.57) cannot parse import attributes at any `ecmaVersion` — `"latest"` caps at ES2024 and this is ES2025 — so `npm run lint` failed where the build did not, which would have blocked the deploy. Rather than take a devDependency for a lint-only problem, `app/i18n/**/*.js` now borrows **`@typescript-eslint/parser`**, which this config already installs and already uses for every `.ts` file, and which reads the syntax via the installed TypeScript 5.9. The override says when it can be removed.
  - **The test had to leave vitest's pipeline.** Vitest runs through Vite, so an in-test `import()` is transformed by the very thing that made the bundle work — it would have passed on the broken code and proved nothing. `tests/utils/workerModuleLoad.test.js` **spawns real `node` processes** and imports each module the scheduler imports, reading that list out of `scheduler.server.js` so a fifth job is covered without anyone remembering. On the previous code three of its five cases fail, naming `ERR_IMPORT_ATTRIBUTE_MISSING` and naming `weeklyReport` as the only one of the four that breaks.
  - **PROVED IN PRODUCTION, on the worker machine, not from a green build.** `/api/health?deep=1` at 12:32:44Z reported `weeklyReport {ok:true}` alongside the other three, `failing: []`. Then the module was loaded the way the scheduler loads it and the function was called with the real clock: **loaded in 298 ms**, returned **`{ran:false, reason:"not the hour"}`** against Sydney weekday 3 / hour 22 versus the report's Monday 09:00, and the Redis week-claim read back `null`, so the probe cannot suppress Monday's real run.

- **DONE — AND THE REASON NOBODY SAW IT FOR WEEKS IS FIXED TOO. False greens #34 and #35 (`982857b`).** `/api/health?deep=1` answers two questions: is a BullMQ worker attached, and are there failed rows in `GenerationJob`. **A scheduler tick that dies inside `import()` reaches neither**, so a job that was 100% dead was indistinguishable from a job that was idle, and both numbers were correct the whole time. The four scheduled jobs now run through `runScheduled()`, which **keeps every existing log line exactly as it was** and records a per-job consecutive-failure count in Redis — Redis because the scheduler runs in the WORKER and the health endpoint answers from a WEB machine, so an in-memory counter is invisible to the thing that has to report it. One `HGETALL` of a four-field hash on the deep path: **three consecutive ticks degrades, ten stops it saying the worker is fine.** The record carries the error's `code` and class and **never its message** — a message can carry a shop domain and this value is read back by a public endpoint; there is a test on that.
  - **A success is RECORDED, not merely an absence.** These jobs return `{ran:false, reason:"not the hour"}` and log nothing for all but one minute a week, so "no error in the log" is equally consistent with working and with failing in a new and quiet way — which is the ambiguity that let this run for weeks. Each tick now writes a moving `{ok:true, at}`, so `checks.scheduler.jobs` is positive evidence readable from outside. **This is how frame-03-style silence gets settled in future: look at whether the timestamp moved.**

- **CC/ALL — two scripts were missing from `scripts/README.md` and `tests/docs/repo-hygiene.test.js` was already RED on `main` before Phase 16 touched anything.** `restore-drill-compare2.mjs` (from the A4 drill, `b591762`) and `verify-client-secret.mjs` (from the 401 outage). A red suite on `main` blocks every deploy, including other workers'. Both are documented now. **Worth noticing as a pattern rather than as two rows:** the guard did its job and told the truth, and nobody was reading it — which is the same disease as the health check above, one layer up.
...
- **ROOT CAUSE FOUND — the 401 is a SIGNATURE failure, not a broken install. The secret Fly holds is not the one Shopify signs with.** Read from `fly logs` 11:29:29Z / 11:29:42Z, 2026-09-16, both `navaal-ttv-03` and `contentpilot-dev2`, identical shape: Shopify calls `/?…&id_token=…` → 302 → `/app?…&id_token=…` → 302 to `/auth/session-token` → 200 → the retry `/app?…` comes back **401** twice. A valid `id_token` would have been accepted on the first call; the bounce means `authenticate.admin` rejected the JWT, and the bounce page's fresh token was rejected too. The same minute, `POST /webhooks/app/subscriptions_update` → **401** — webhook HMAC is computed with the same client secret. Two independent surfaces, one shared input. `aud` in the token is `1279a14cca41d4a6f8e6e3c485870b77`, which equals `client_id` in `shopify.app.toml`, so `SHOPIFY_API_KEY` is correct and only `SHOPIFY_API_SECRET` is wrong. **The rollback import put back the secret that was revoked; Shopify now signs with the one that survived.** Fix is to import the surviving (active) secret. No reinstall, no `shopify app deploy`, no new app version.
- **TOOL — `scripts/verify-client-secret.mjs`: never guess a secret again; prove it before importing.** Takes a candidate secret file and a log file, pulls the newest real `id_token` out of the log, recomputes its HMAC-SHA256 and prints **MATCH** or **NO MATCH**. Prints the shop, the `aud`, the secret's *length*, and the last four characters of the *expected* signature — never the secret, never the token. Exit 0 on match, 1 on mismatch, 2–5 on operator error with the command that fixes it. Tested on the owner's machine against `applog.txt` with a deliberate wrong value: correctly NO MATCH, `aud` confirmed equal to `client_id`. **Standing rule, extended: a secret is verified against a real signature before it is imported, not after the app is down.** Also extended: `applog.txt` and `*.flylog` are now gitignored — raw logs carry live id_tokens and real shop domains.
- **CC — PRODUCTION DEFECT, throwing once a minute since the i18n work landed: `app/i18n/catalogues.server.js` cannot be loaded by Node at runtime.** From the same log, every 60 s on machine `d8d996d7b1ed28`: `TypeError [ERR_IMPORT_ATTRIBUTE_MISSING]: Module "file:///app/app/i18n/locales/de.json" needs an import attribute of "type: json"` → `"msg":"weekly report threw"` (`scheduler.server.js:403`). Lines 14–19 of `catalogues.server.js` are bare `import de from "./locales/de.json"` (and the six Polaris locales below them); `app/i18n/chunks.js:13` does the same through dynamic `import()`. Vite resolves these at build time in the app bundle, but the scheduler path loads the module through Node's own ESM loader, where a JSON import without `with { type: "json" }` is a hard error. Consequence: **the weekly report has not run in production at all**, and any other runtime path that reaches the catalogues server-side fails the same way. Fix: add `with { type: "json" }` to every JSON import in `catalogues.server.js` and `chunks.js`, or read them with `fs.readFileSync` + `JSON.parse` — then prove it by a clean minute of `fly logs` with zero `weekly report threw`, not by a green build. **This is why nobody noticed: `/api/health?deep=1` reports `worker running` and `failedLast10Min 0` — the throw is caught by `.catch()` and logged, so the health check stays green while the job never runs. Add the weekly-report throw to whatever the deep health check counts, or it will hide again.**

- **A4 — PROVED IN FULL, 2026-09-16. The branch was read, and it holds the data.** The owner added a compute (`restore-drill-temp`, 0.25 CU, scale-to-zero) to `restore-drill-2026-09-16T0127Z` and ran the comparison; Cowork diffed the two sides. **Branch 23 tables / 337 columns / 18 non-empty; production 23 / 337 / 18. Zero columns present on one side and absent on the other. Twenty tables identical.** The three that differ are all *lower* on the branch and all append-only, which is the drift the branch's point-in-time predicts: `LogEvent` 4,375 vs 4,879, `ProductScore` 98 vs 120, `CrawlerAccess` 54 vs 58. **Zero tables empty on the branch and full on production — the script's own definition of a failed restore, and it did not occur.** Evidence: `docs/history/restore-drill-2026-09-16-production.json` and `docs/history/restore-drill-2026-09-16-branch.json`, branch host `ep-aged-mud-a7vov7fd-pooler.ap-southeast-2.aws.neon.tech`. A4's line — *"a second person could repeat it from the runbook"* — is now met by construction: a second person did, from CC's write-up, on a different machine, and the numbers agreed.
- **Tooling note for `07-VERIFICATION.md` — the compare script's first cut could not run on Windows, and the reason is worth keeping.** `new PrismaClient({ datasources: { db: { url } } })` does not stop Prisma validating `env("DATABASE_URL")` at construction, so the run died three times with a schema-validation error that named a variable the operator had already set, while the variable they *were* told to set was ignored. Rewritten as `scripts/restore-drill-compare2.mjs` (`b591762`): the connection string is read **from a file** — nothing typed into a shell, nothing in shell history, no quoting rule to defeat it — `DATABASE_URL` is set in `process.env` before the client is imported, `datasourceUrl` is used rather than the deprecated `datasources`, and the three paste shapes people actually produce (Neon's `psql '<url>'` line, a quoted string, a `KEY=value` line) are repaired rather than rejected. **The rule: when a step fails twice on the operator's machine, fix the tool, not the operator's typing.**
- **OWNER, last step on A4 — delete the drill branch now that it has been read.** Neon console → project `contentclaude` → **Branches** → `restore-drill-2026-09-16T0127Z` → delete. That removes the `restore-drill-temp` compute with it. A drill that leaves litter is half a drill, and this one is finished.

- **FRAME 01 — FR8 IS GENUINELY FIXED, AND THE FRAME STILL CANNOT SHIP. Cowork did the harness's own "NOW LOOK AT THEM" step, 2026-09-16.** The new capture (`listing-assets/01-home-desktop.png`, 105,540 bytes, 1600×900, from `7710be2`) reads: store `65 / 100 across 14 products sampled`, and the three rows now say **`At first run: 56/100`** under the sentence *"These 3 products all score 56: they are missing the same things, so each one's number is the same as the store's. We scanned 14 products to pick them."* Two different numbers, both correctly labelled — exactly what CC set out to do, and the screen is honest where it used to lie. **But the card also carries: _"Your storefront is password-protected, so nothing can be listed anywhere yet. Expected before launch; we re-check the night it opens."_** That line is true, useful, and completely correct in the app — and on a listing screenshot it tells every prospective merchant that the store in the picture is not open for business. **It is a store problem, not a screen problem, and it will appear on EVERY frame we capture, because every capture store is a development store and every development store is password-walled** (F10, proved on three surfaces). Do not "fix" it in the app: suppressing an honest advisory to make a marketing image look better is the thing this project refuses to do. **The real route is F10's:** capture the listing frames on a store whose storefront is actually open — `elitepepsaustralia.com` if it is a live Shopify store the owner controls, or a new store on the $1/month plan. Until then the English gallery keeps its three current frames (Review, Products, Settings), which carry no such line, and frames 01 and 04 stay held — now for this reason rather than FR8's.
...
- **DONE — THE TWO PLAN-INTEGRITY DEFECTS ARE FIXED, AND READING THEM FOUND A THIRD THAT WAS WORSE (CC, Phase 15, `e3e5952`).** Cowork's judgement was right: they were not confusions. `14-PRICING.md` §4 is the owner-approved table, so the table is the side that was right and the code is what changed. Each fix has a test that goes red on the old behaviour — **8, 6, 2, 3 and 4 failures** across the five reverts.
  - **(a) Bulk on Free — and the premise was half wrong in our favour and half wrong against us.** The first screen's dark button did **not** lead where the plan could not go: **`/app/fix` had no entitlement check at all.** `bulkJobs` is enforced in Products and in Optimize, which is exactly why "bulk is gated" read as true — but the fix page would apply a vendor to fifty products or queue fifty descriptions on a **free** store, against a plans table that shows **Bulk runs ✗** for Free. Five of its six intents are now gated server-side with the existing `bulkRefusal` sentence, and the page states the gate **before** the press instead of on it. The sixth, `gtin_exempt`, is deliberately **not** gated: it writes a preference to our own database ("own brand — stop asking"), touches no catalogue, costs nothing, and charging a merchant to stop being nagged would be the wrong direction — so it is also the one fix a free store can always press.
  - **(b) The first screen no longer dead-ends.** Every blocker fix that goes to `/app/fix` is marked `needsBulk`, and two pure rules in `firstRun.js` decide the screen: a fix this plan cannot run is never the primary control, and it carries a marker naming the plan that can (`Starter and above`, derived from the table, never typed). When the plan can run none of them, **nothing is primary** — a dark button that refuses is worse than no dark button.
  - **(c) Blog is gated where the table sells it, and the price is on the button that spends it.** A `blogPosts` entitlement carries the locked table into the code (Free no, Starter no, Growth yes, Pro yes). The action refuses **before** a credit is reserved and before the model is called; the screen, when the plan does not carry blog, names the price, the plan, and that posts already written stay and can still be published; the comparison table gained a blog row derived from the same entitlement with its number read from `CREDIT_WEIGHTS`.
  - **Who loses something: nobody.** Read from production 2026-09-16 — exactly **one** shop has ever written a blog post (`contentpilot-dev2`, 6 posts, **on Pro**, which keeps it). Plan census: **16 free / 1 pro**.
  - **And the 3-credit price had never once been charged.** All six of those rows are dated **2026-08-12 at 1 credit each**, a month before the weighting shipped (`0c5274b`, 2026-09-14). No blog post has ever been billed at 3 in production. That is why `tests/utils/blogCredit.test.js` **runs the real serializable transaction** with only Prisma doubled and reads the row that comes out (`contentType blog, credits 3`), rather than reading the weight table and calling it proof — plus the boundary (2 left refuses and reports that a post needs 3) and the trial bucket incrementing by 3.

- **DONE — FRAME 03: THE ROW, NOT THE LABEL. It is `gid://shopify/Product/7800250007655` on `contentpilot-dev2` (CC, `e3e5952`).** Our state **published**, Shopify status **DRAFT**, `publishedAt` null, no online-store URL. Found read-only by walking both scopes and diffing them: the list walk returns **15**, the candidate walk **14**, and that product is the difference. **Two populations were on one screen** — the list is `-status:archived` (wide on purpose: a merchant's drafts are still theirs) while the header counts the candidate scope, which for this shop is `(status:active OR status:draft) AND published_status:published`. The tabs' only qualifier was **"on this page"**, which explains *pagination*; this store's 15 products fit on one page, so it explained nothing at all. Fixed: the list query now asks Shopify for `publishedAt`, the row carries it, a line under the tabs names the difference when the page holds one, and **the row badge now keys on the same fact the count does** — which also catches a product that is ACTIVE but unpublished from the channel, a case the old status-only badge missed entirely. **CW: frame 03 is unblocked for re-capture** — the screen now reconciles, and 17 further content rows sit on ARCHIVED products, which is the already-explained lifetime-vs-scope gap, not a new one.

- **DONE — FR14's ACCESSIBLE LABEL IS VERIFIED HERE NOW, NOT LEFT ON CW (`e3e5952`).** No harness can actuate inside this app's cross-origin admin iframe, so the half that matters was never checked: does Polaris's `ariaLabelledBy` reach the element, and is the fraction what the bar is *named* by? A source check cannot answer it — if Polaris ignored the prop, every Phase 14 assertion would still pass and the bar would still announce "1%". `tests/routes/quotaAccessibleName.test.js` renders Polaris and reads the markup: the prop **does** emit `aria-labelledby` on a real `<progress>`, with no `aria-label` or `aria-valuetext` competing, and the name resolves to the fraction on all four surfaces. **One thing to know, found while there:** Polaris also emits its own `<span class="Polaris-ProgressBar__Label">19%</span>`, hidden by `clip-path: inset(50%)` — visually gone, still in the accessibility tree. It is Polaris's, not ours, and it is no longer the bar's name; the test pins both, so a Polaris change is found here rather than on a listing screenshot.

- **CW/OWNER — what is still owed on these, and it is not code.** (1) **Frame 03's re-capture** on `contentpilot-dev2`, now that the screen reconciles. (2) **`node tools\proof\listing-assets.mjs 01-home` on Windows** — unchanged from the row above; CW's Linux VM is missing `libXdamage.so.1`. (3) **FR13's one owner click** — unchanged: press `Review` on a Ready-to-review row and read the address bar for `?product=`. Nothing in this phase touched any of the three.
