...
|---|---|---|---|
| P0.1 | CC | **VERIFIED OPEN 2026-09-13, 18 days left.** `extensions/geo-schema/shopify.extension.toml` declares **no `api_version` at all** — only `name`, `type` and `uid`. So the extension's effective version is whatever was current at its last deploy, and we cannot see it from the file. Find the deployed version, pin it explicitly to 2025-10 or later, and redeploy the extension. Verify the theme app extension's API version is **2025-10 or later**. Extensions on 2025-07 or earlier **cannot be updated** after this — we would be frozen out of our own storefront code. | **1 Oct 2026** |
| P0.2 | CC | **VERIFIED CLEAN 2026-09-13** — the only match in the codebase is `jsonLdScriptTag()` in `app/utils/geo.server.js:239`, a helper that builds a JSON-LD `<script>` string; nothing calls Shopify's ScriptTag API. Re-verify before 1 Oct. Prove we create **no script tags**. `scriptTagCreate`/`Update` begin returning user errors; *"applies to all API versions, including older ones, so pinning won't defer it."* | **1 Oct 2026** |
| P0.3 | CC | **VERIFIED MISSING 2026-09-13** — there is no version check anywhere in `ci.yml`. Admin API is pinned to **2026-04** (`shopify.app.toml:14`), good to Apr 2027. **Add a CI check that fails when the pin is within 90 days of sunset.** An app using unsupported resources is **delisted** and installs are blocked for at least seven days. | now |
