...
| | **Status 2026-09-15:** layer live `1d9d8f7` (D0); **de live `a9ffd38`** (D1, 1,365 keys, read back on production); **fr live `86b417c`** (D2, read back on production); **es live `36d95ee`** (D3, read back on production; the same deploy moved every date on a merchant screen to the screen's locale); **it live `391feb6`** (D4, read back on production); **pt-BR live `33cfde1`** (D5, read back on production); **ja live `23423bb`** (D6, read back on production with the layout check; the check found two English sentences on every language and the scanner rules written for them found 47 more — all keyed at `4d32d98`). **All six live; B2 is done.** | |
...
| C3 | Listing in six languages | **All six gates met — de `a9ffd38`, fr `86b417c`, es `36d95ee`, it `391feb6`, pt-BR `33cfde1`, ja `23423bb`. CW enters them now; C3 closes when CW's six counts are posted.** | CW |
...
| C10 | The weekly report to merchants — the heartbeat that earns the review | **CORRECTED 2026-09-16 (CC, Phase 16, `982857b`): it was never live.** This row said "live; never fired (no merchant with a result yet)" — a reason that fitted the silence and was not the reason. The scheduler imports `weeklyReport.server.js` every 60 s; that chain reaches `app/i18n/catalogues.server.js`, whose bare JSON imports **Node's own ESM loader refuses**, and the worker loads source with Node rather than the Vite bundle. `maybeSendWeeklyReports()` had therefore never executed once. Now it does: proved on the worker machine at 12:33Z — module loaded in 298 ms, returned `{ran:false, reason:"not the hour"}` against Sydney weekday 3 / hour 22 versus the report's Monday 09:00, week-claim untouched. **Still never fired**, which is now a statement about Mondays and merchants rather than about a module that could not load. | — |
