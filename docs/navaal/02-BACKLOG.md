...
| ID | Owner | Item | Status |
|---|---|---|---|
| C0.5 | CC | **B5 — the annual one-time 2× credit month.** `14-PRICING.md` §5 calls it the best-value line in the table: it costs $17.25 on Growth against $287.90 already collected and solves the merchant's real problem, the initial burst, in exchange for the twelve-month commitment. The 20% annual PRICING is shipped (`2d21956`); the boost is not. It must be **structurally** once-ever — a stamp on `Shop`, which survives uninstall, like `trialUsedAt` — and not a flag anyone can flip. **Not urgent in the ordinary sense and deliberately not rushed: there are zero annual subscribers, so nothing is being denied today. It must exist before annual is sold.** | OPEN |
| C0.6 | CC | **B6 — credit packs (1,000/$19 · 2,000/$39 · 4,000/$79).** ROUTED rather than half-built, which `14-PRICING.md` §6 item 6 and the Phase 4 brief both explicitly permit. They require the **Billing API**, not App Pricing: App Pricing does not support one-time purchases and since **28 Apr 2026** sends no `APP_SUBSCRIPTIONS_UPDATE` webhook and no `charge_id`. **Confirm which billing path this app is on before starting** — if it is App Pricing today, packs are a larger change than the other seven B items combined and deserve their own phase. Two hard requirements when built: packs are consumed **after** the monthly allowance, never before, and a purchase must be **idempotent against webhook redelivery** — credits granted twice for one payment is the failure that ends the app's reputation. | OPEN |
| C0.7 | CC | **B7 — bring-your-own AI key at Pro.** Does not exist in code or schema; a build, not a toggle. Requirements that are not negotiable: the key is stored **encrypted** and **never logged, never returned to the client, never in an error message — not the key, not a prefix, not a length** (L9); it is **validated on save with one real cheap call**, because a key that fails at 2am mid-bulk-job is a support ticket and a refund; and if it fails mid-job the job **pauses and tells the merchant** rather than silently falling back to our key and our money. One decision to make and record in `04-DECISIONS.md` first: whether generations on a merchant's own key still consume credits for accounting. Pick one and make the UI say which. | OPEN |
| C0.4 | CC | **Every page load throws ~12 uncaught `pageerror`s, and it is our own markup.** Found 2026-09-14 while capturing the rendered document for P0.4, from a real authenticated `/app` load on `contentpilot-dev2`: *"Unexpected tag <<div slot=\"logo\" style=\"padding:8px 16px\">..."*, repeated **11–12 times per load**. `app/routes/app.jsx:174` puts a `<div slot="logo">` inside `<s-app-nav>`, which is a Shopify **web component** and rejects an arbitrary `<div>` as a child. **Pre-existing** — present in both the before and after captures, so P0.4 did not cause it. Uncaught exceptions on every load are a quality signal a BFS reviewer can open the console and see, and the brand logo may not be rendering at all. Re-read it with `node tools/proof/appbridge-head.mjs`. | OPEN |
