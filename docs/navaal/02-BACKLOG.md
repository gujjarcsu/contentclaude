...
| ID | Owner | Item | Status |
|---|---|---|---|
| F1 | CC | **✅ DONE 2026-09-15 (Phase 10 Part C), live at `da265c4`.** The matrix is data (`tests/fixtures/shapeMatrix.js`: 36 rows over every §4 axis × 9 phases) and `tests/utils/shapeMatrix.test.js` runs every PASS cell through the REAL function — candidate predicate, content classifier, first-run scan via shopifyQuery with retries off, the walk's grader with F3's second look, plan fit — and refuses a PASS cell without an assertion or an assertion without a PASS cell. **PASS 107 · HELD 15 · NOT RUN 75**, every NOT RUN naming where it goes; `docs/navaal/SHAPE-MATRIX.md` carries the table verbatim under a doc test. Six shapes the first fixture file never built (empty store, barcodes on later variants, 100 variants, multipacks, above the Free cap, Translations) plus the plan and API axes as contexts. **Found and fixed on the first run:** ALL_DRAFT, B2B_ONLY and ACTIVE_NOT_PUBLISHED scan as EMPTY (correctly — scoped to Active on the Online Store) and the empty screen told a merchant with twenty products to "add a product"; Home now passes the counts and the screen says *"Your products aren't on your Online Store yet"* with the three reasons. Three observations recorded, not changed (§2 of the doc). The original brief, kept: Store-shape fixture matrix over every axis in `05-EVIDENCE.md` §4. Report per phase which cells you proved and which you did not. **A cell you did not test is a defect you have not found yet.** | DONE |
| F2 | CC → CW | **Catalogues built 2026-09-15; the six stores are queued for CW.** `scripts/shape-csv.mjs` generates the five `navaal-shape-*` catalogues as Shopify product-import CSVs from the same fixtures the matrix runs (`tools/proof/fixtures/shapes/`; CSV import publishes to the Online Store and creates variants with barcodes without the `write_publications` scope this app deliberately lacks — the path that built the ttv stores). `tests/utils/shapeCsv.test.js` holds each file to its generator and its shape. Names: `navaal-shape-drafts` · `-variants` · `-fr` · `-b2b` · `-cap` · `-zero` (no import) — recognised by every dev-store guard and by the funnel's test-shop exclusion; none captured, none frozen, none EBS. Creating a development store is a Partner Dashboard action: runbook in SHAPE-MATRIX.md §3 (create → import → install LAST → post the handle), then First-run scores + read-screen per store, and one Review publish on `-variants` and `-fr` closes their Draft/Publish cells. Original: One real dev store per shape that matters most: all-draft · variant-heavy · non-English · B2B-only · catalogue above the plan cap. Fixtures model reality; a real store *is* reality. **CW 2026-09-15: all six stores built, imported and installed (install last, every time). Handles, counts, publish state and first screens in `06-QUEUE.md` Phase 11.** Three findings out of it: `navaal-shape-cap` says `we scanned 30 of your products` beside three counts of `150`; the AI writes English page titles and descriptions on the all-French store (accents survive, the language does not); `Mitigeur de cuisine 0` loses its `0`. One correction: these six close **61** of the 90 routed lines, not 75 — 29 name stores none of these is. | DONE (CW) → CC |
...
| F9 | OWNER | **CORRECTED 2026-09-16 (Phase 14 item 7): there is nothing to apply for.** This said P3.4 was blocked on "Shopify Level 2 protected customer data approval (+ `read_reports`)" and told the owner to start an application. CW read the Partner API-access page in the owner session: it offers eight requests and **none is `read_reports`**, so no such form exists and P0.10 is closed as a false premise. `read_reports` is an ordinary scope; adding it re-prompts every installed merchant for consent, so it is governed by the standing scope decision — **no new scopes until ten merchants** — and this row waits on merchant count, not on Shopify. The classification and labelling are built and tested; the query is one loader once the scope is added. | OPEN — on ten merchants, not on an application |
