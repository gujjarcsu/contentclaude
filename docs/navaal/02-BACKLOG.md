...
| A4.2 | Copy and behaviour must agree. Home promises "every product missing a description"; the button targets everything without OUR content. Fix the behaviour; put "rewrite existing" behind a separate labelled action. | OPEN |
...
| B6 | Dev-store residue sweep: snowboard, ski, snow, wax and every Shopify demo example. Test that fails on those strings in user-facing copy. | DONE 2f8dd98 — done OUT OF PHASE ORDER while in Phase A; flagged rather than hidden. |
