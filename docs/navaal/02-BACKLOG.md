...
| ID | Item | Status |
|---|---|---|
| INFRA1 | Deduplicate deploys. `ci.yml` on push + `deploy.yml` on dispatch; `concurrency` serialises without deduping. | DONE (live proof pending) — BOTH paths now skip a commit that is already live. `deploy.yml` gains a `force` input for repairing a broken deploy and a post-deploy confirmation step; `ci.yml` needs no force because a re-run after a FAILED deploy self-selects (build-info still reports the old commit). **Fails safe**: an unreadable build-info deploys rather than skips — refusing to deploy because we could not check is how a broken production stays broken. Logic proved against four cases. |
