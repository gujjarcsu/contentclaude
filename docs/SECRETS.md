...
| `REMEDIATION_LOCKED_SHOPS` | Comma-separated shop domains that are **monitored but never written to** from the bulk-fix page (P2.6). Set by the owner, from a file, never typed here. | Nothing is locked: every installed shop can use Fix in bulk. The daily watch is read-only regardless. |
| `MERCHANT_EMAIL_FROM` | Sender for the one merchant-facing email (the P3.6 weekly report), e.g. `Navaal <hello@navaal.ai>`. Must be a Resend-verified domain. | Defaults to `Navaal <hello@navaal.ai>`; the report still sends. |
...
| `BYOK_ENCRYPTION_KEY` | 32 random bytes, base64. Encrypts merchants' own Anthropic keys (AES-256-GCM, `app/utils/secretBox.server.js`) | **Bring-your-own-key is OFF, and fails CLOSED:** `isEnabled()` returns false, the Settings card does not render, and `encrypt()` throws rather than storing anything. Nothing else in the app is affected. There is deliberately **no fallback key** — deriving one from the source tree or reusing `SHOPIFY_API_SECRET` would mean merchants' credentials protected by a value an attacker with the repo already has. A wrong-LENGTH value is treated the same as absent, for the same reason. |
...
| `BYOK_ENCRYPTION_KEY` | **Rotating this DESTROYS every stored merchant key.** They become undecryptable, `resolveKeyFor` treats that exactly like an auth failure — it blocks rather than silently falling back to our key and our money — and every affected Pro merchant must paste their key again. There is no re-encryption path and building one would mean holding every merchant key in memory at once. **Generate and install it ONLY through the `BYOK encryption key — one-time init` workflow**, which creates it on the runner, pipes it straight into `flyctl secrets import` from a file, shreds the file, and refuses to run a second time unless the rotation is confirmed by typing an exact phrase. The value is never printed, never an argument, and never seen by a human — including the owner. |
