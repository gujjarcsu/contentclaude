...
const STORE = "navaal-test-2", APP = "navaal-seo-geo-content";
const ADMIN = `https://admin.shopify.com/store/${STORE}`;
const APP_ROOT = `${ADMIN}/apps/${APP}`;
const OUT = "title-proof2", FRAMES = `${OUT}/frames`;
...
if (!b) { console.log("INCOGNITO DEAD"); process.exit(1); }
const ctx = b.contexts()[0];
let page = ctx.pages().find((p) => /admin\.shopify\.com\/store\/navaal-test-2/.test(p.url())) || ctx.pages()[0];
const appFrame = () => page.frameLocator('iframe[src*="navaal"], iframe[src*="app.navaal.ai"]').first();
const log = (m) => console.log(`[r] ${m}`);
const results = [];
page.on("response", (r) => { try { const u = new URL(r.url()); if (u.hostname === "app.navaal.ai" && ["/", "/app"].includes(u.pathname) || (u.hostname === "app.navaal.ai" && (u.pathname.startsWith("/auth/login") || u.pathname.startsWith("/reembed")))) log(`  ← ${r.status()} ${u.pathname}${r.headers()["location"] ? " → " + r.headers()["location"] : ""}`); } catch { /* */ } });
...
const go = async (u, cap) => { if (cap) await caption(cap); await page.goto(u, { waitUntil: "domcontentloaded" }).catch(() => {}); await appFrame().locator("body").waitFor({ state: "visible", timeout: 40000 }).catch(() => {}); await page.waitForTimeout(3800); };

let ok = true;
try {
  const capP = capture();
  await go(`${APP_ROOT}/app/products`, "Fresh dev store · incognito · on the Products feature page"); if (await check("1-products")) ok = false;
  // TITLE CLICK equivalent: load the app ROOT (application_url "/").
  await go(APP_ROOT, "Click the app NAME → loads the app root '/' (the reviewer's exact move)"); await caption("→ recovers to the Dashboard, NO login form"); await page.waitForTimeout(1500); if (await check("2-app-title-root")) ok = false;
  for (const [nm, p] of [["Products", "/app/products"], ["Optimise Store", "/app/optimize"], ["Plans & Billing", "/app/plans"], ["SEO Audit", "/app/seo-audit"], ["Settings", "/app/settings"], ["Dashboard", "/app"]]) { await go(`${APP_ROOT}${p}`, `Nav → ${nm}`); if (await check(`nav-${nm}`)) ok = false; }
  await go(`${APP_ROOT}`, "App name again from a sub-page"); if (await check("3-app-title-again")) ok = false;
  await caption("Browser Back button"); await page.goBack({ waitUntil: "domcontentloaded" }).catch(() => {}); await page.waitForTimeout(3000); if (await check("browser-back")) ok = false;
  await caption("App name (root '/'), every nav item, browser back — the login form never appears ✓"); await page.waitForTimeout(3000);
