...
const check = async (label) => {
  ...
  for (let i = 0; i < 26; i++) {
    try {
      ft = await appFrame().locator("body").innerText({ timeout: 2500 });
    } catch {
      ft = "";
    }
    if (
      /Welcome back|Monthly Usage|Choose Your Plan|Brand voice|Get started|Shop domain|>Log in<|name="shop"/i.test(
        ft,
      )
    )
      break;
  ...
  log(
    `${form ? "❌ FORM" : ok ? "✅ ok" : "· "} — ${label} | form=${form} len=${ft.trim().length} | ${page.url()}`,
  );
  await page
    .screenshot({
      path: `${OUT}/${String(results.length).padStart(2, "0")}-${label.replace(/[^a-z0-9]+/gi, "_").slice(0, 30)}.png`,
    })
    .catch(() => {});
  return { form, ok };
};
...
try {
  ...
  if ((await check("1-open-dashboard")).form) ok = false;
  ...
  if ((await check("3-dashboard-via-nav")).form) ok = false;
  log("reload /app (reviewer kill move)");
  await page.reload({ waitUntil: "domcontentloaded" }).catch(() => {});
  await page.waitForTimeout(3500);
  if ((await check("4-reload-app")).form) ok = false;
  if (!(await clickNav("Settings"))) await gotoTop(`${APP_ROOT}/app/settings`);
  if ((await check("5-settings")).form) ok = false;
  if (!(await clickNav("Dashboard"))) await gotoTop(`${APP_ROOT}/app`);
  ...
  await page.reload({ waitUntil: "domcontentloaded" }).catch(() => {});
  await page.waitForTimeout(3500);
  if ((await check("7-reload-plans")).form) ok = false;
  // app name / home (loads application_url "/")
  await gotoTop(`${APP_ROOT}`);
  if ((await check("8-app-name-home")).form) ok = false;
} catch (e) {
  log("ERROR: " + e.message);
  ok = false;
}
