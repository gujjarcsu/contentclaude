...
let cookieStr = "";
try {
  cookieStr = await frameOf().evaluate(() => document.cookie);
} catch (e) {
  cookieStr = "(unreadable: " + e.message + ")";
}
...
try {
  await frameOf().evaluate(() => window.location.assign("/app"));
} catch (e) {
  log("force err: " + e.message);
}
...
for (let i = 0; i < 30; i++) {
  ...
  try {
    frameText = await appFrame().locator("body").innerText({ timeout: 2000 });
  } catch {
    frameText = "";
  }
...
console.log(
  form
    ? "❌ FORM SHOWN"
    : recovered
      ? "✅ RECOVERED to the app (no form)"
      : "⚠️ no form but did not clearly recover",
);
try {
  browser.close();
} catch {
