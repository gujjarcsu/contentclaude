...
const context = await browser.newContext({ storageState: "tests/e2e/.auth/navaal-test-2.json", viewport: { width: 1440, height: 900 } });
...
const capture = async () => { while (running) { try { await page.screenshot({ path: `${FRAMES}/f-${String(n).padStart(5, "0")}.jpg`, type: "jpeg", quality: 70 }); n++; } catch { /* */ } await new Promise((r) => setTimeout(r, 220)); } };
const check = async (label) => { let t = ""; for (let i = 0; i < 22; i++) { try { t = await appFrame().locator("body").innerText({ timeout: 2500 }); } catch { t = ""; } if (/Welcome back|Monthly Usage|Choose Your Plan|Brand voice|Shop domain|>Log in</i.test(t)) break; await page.waitForTimeout(1000); } const form = /Shop domain|name="shop"|>Log in</i.test(t); results.push({ label, url: page.url(), form }); log(`${form ? "❌ FORM" : "✅ ok"} — ${label} | ${page.url()}`); return form; };
const gotoTop = async (u) => { await page.goto(u, { waitUntil: "domcontentloaded" }).catch(() => {}); await appFrame().locator("body").waitFor({ state: "visible", timeout: 40000 }).catch(() => {}); await page.waitForTimeout(3500); };
const clickTitle = async () => {
  ...
  for (const c of cands) { const el = c.first(); if (await el.count().catch(() => 0)) { try { await el.click({ timeout: 6000 }); return true; } catch { /* next */ } } }
  return false;
};

let ok = true;
try {
  await page.goto("https://app.navaal.ai/api/build-info", { waitUntil: "domcontentloaded" }); await page.waitForTimeout(1500);
  const capP = capture();
  await caption("Live build SHA"); await page.waitForTimeout(2000);
  await gotoTop(`${APP_ROOT}/app/products`); await caption("On a feature page (Products)"); if (await check("1-products")) ok = false; await page.waitForTimeout(1500);
  await caption("Click the APP NAME at the top of the sidebar (the reviewer's exact move)");
  const clicked = await clickTitle();
  log("title clicked: " + clicked); await page.waitForTimeout(6000);
  await caption("→ URL settles on /app, dashboard renders — NO login form"); if (await check("2-after-title-click")) ok = false; await page.waitForTimeout(2500);
  // every nav item
  for (const [nm] of [["Products"], ["Optimise Store"], ["Plans & Billing"], ["Settings"], ["Dashboard"]]) {
    await caption(`Nav: ${nm}`);
    const l = appFrame().getByRole("link", { name: new RegExp("^" + nm.replace(/[.&]/g, ".") + "$", "i") }).first();
    try { await l.click({ timeout: 5000 }); } catch { /* */ }
    await page.waitForTimeout(2200); if (await check(`nav-${nm}`)) ok = false;
  }
  // Plans back arrow
  await caption("Plans page back arrow");
  await gotoTop(`${APP_ROOT}/app/plans`);
  const back = appFrame().locator('button[aria-label*="Back" i], a[aria-label*="Back" i], [class*="Breadcrumb"] a').first();
  try { await back.click({ timeout: 4000 }); } catch { /* */ } await page.waitForTimeout(2500); if (await check("plans-back-arrow")) ok = false;
  // browser back
  ...
  await caption("App title, every nav item, back arrow, browser back — no login form ✓"); await page.waitForTimeout(2500);
...
if (files.length) {
  const ff = spawn(FF, ["-y", "-f", "image2pipe", "-vcodec", "mjpeg", "-framerate", "5", "-i", "pipe:0", "-c:v", "libvpx", "-b:v", "2M", "-pix_fmt", "yuv420p", "-vf", "scale=1440:-2", `${OUT}/title-click-proof.webm`], { stdio: ["pipe", "ignore", "ignore"] });
  ff.on("close", () => { const p = `${OUT}/title-click-proof.webm`; console.log(fs.existsSync(p) ? `VIDEO: ${p} (${(fs.statSync(p).size / 1e6).toFixed(1)}MB)` : "NO VIDEO"); process.exit(0); });
