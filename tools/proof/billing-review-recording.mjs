...
const waitFile = async (p, timeoutMs = 180000) => {
  const t0 = Date.now();
  while (Date.now() - t0 < timeoutMs) { if (fs.existsSync(p)) return true; await new Promise(r => setTimeout(r, 500)); }
  return false;
};
...
const CURSOR = () => {
  ...
  const install = () => {
    ...
    try { const s = JSON.parse(localStorage.getItem("__vcpos") || ""); if (s && typeof s.x === "number") last = s; } catch {}
    const dot = document.createElement("div");
    dot.id = "__vcdot";
    Object.assign(dot.style, {
      position: "fixed", width: "16px", height: "16px", borderRadius: "50%",
      background: "#111", border: "3px solid #fff", boxShadow: "0 0 6px rgba(0,0,0,.7)",
      zIndex: "2147483647", pointerEvents: "none", transform: "translate(-50%,-50%)",
      left: last.x + "px", top: last.y + "px", transition: "none",
    });
  ...
  window.__vcMove = (x, y, ms = 400) => new Promise((resolve) => {
    install();
    ...
    const step = (now) => {
      ...
      const e = ease(t);
      const cx = start.x + (x - start.x) * e, cy = start.y + (y - start.y) * e;
      dot.style.left = cx + "px"; dot.style.top = cy + "px";
      if (t < 1) requestAnimationFrame(step);
      else { window.__vcpos = { x, y }; try { localStorage.setItem("__vcpos", JSON.stringify({ x, y })); } catch {} resolve(); }
    };
  ...
  window.__vcRipple = (x, y) => {
    ...
    Object.assign(r.style, {
      ...
      borderRadius: "50%", border: "3px solid #111", zIndex: "2147483646",
    ...
    requestAnimationFrame(() => { r.style.width = "60px"; r.style.height = "60px"; r.style.opacity = "0"; });
  ...
  window.__vcCaption = (text) => {
    ...
    if (!el) {
      el = document.createElement("div"); el.id = "__vccap";
      Object.assign(el.style, {
        position: "fixed", left: "0", right: "0", top: "0", zIndex: "2147483645",
        pointerEvents: "none", font: "600 16px/1.4 -apple-system,Segoe UI,Roboto,sans-serif",
        color: "#fff", background: "linear-gradient(180deg,rgba(17,17,24,.94),rgba(17,17,24,.78))",
        padding: "10px 18px", textAlign: "center", letterSpacing: ".2px",
      });
...
async function caption(page, t) { await page.evaluate((t) => window.__vcCaption && window.__vcCaption(t), t).catch(() => {}); }
...
const browser = await chromium.launch({
  ...
  args: ["--disable-blink-features=AutomationControlled", "--no-default-browser-check", "--no-first-run", "--start-maximized"],
});
...
try {
  ...
  if (await cancel0.isVisible().catch(() => false)) {
    ...
    await cancel0.click(); await page.waitForTimeout(8000); await gotoPlans(page);
  }
  ...
  await caption(page, "Plans & Billing — Free plan. A merchant can change plans here (no support, no reinstall).");
  ...
  f = appFrame(page);
  await f.getByRole("button", { name: /upgrade to professional/i }).first().click();
  ...
  const approve = page.getByRole("button", { name: /^approve/i }).or(page.getByRole("link", { name: /^approve/i })).first();
  ...
  if (await cancel1.isVisible().catch(() => false)) { await cancel1.click(); await page.waitForTimeout(8000); await gotoPlans(page); }
  ...
  if (vids.length) fs.renameSync(`${OUT}/video/${vids[vids.length - 1]}`, `${OUT}/billing-review-recording.webm`);
