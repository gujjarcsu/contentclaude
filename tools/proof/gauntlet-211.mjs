...
const NAV = [
  ...
  ["Review & Publish", "/app/review"],
  ["SEO Audit", "/app/seo-audit"],
  ["Blog Generator", "/app/blog"],
...
const assertHealthy = async (label) => {
  ...
  for (let i = 0; i < 12; i++) {
    try {
      frameText = await appFrame(page).locator("body").innerText({ timeout: 3000 });
    } catch {
      frameText = "";
    }
  ...
  log(
    `${pass ? "PASS" : "FAIL"} — ${label} | url=${url} | form=${formShown} onLogin=${onLogin} contentLen=${frameText.trim().length}`,
  );
  await page
    .screenshot({
      path: `${OUT}/${String(results.length).padStart(2, "0")}-${label.replace(/[^a-z0-9]+/gi, "_").slice(0, 40)}.png`,
    })
    .catch(() => {});
  return pass;
};
...
const clickNav = async (label) => {
  ...
  const link = f
    .getByRole("link", { name: new RegExp("^" + label.replace(/[.&]/g, ".") + "$", "i") })
    .first()
...
try {
  ...
  await caption(
    "Step 3b — click the app name / home (loads the app root “/”). Dashboard loads, no login form.",
  );
  await gotoTop(APP_ROOT);
