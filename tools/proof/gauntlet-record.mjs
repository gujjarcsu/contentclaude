...
const CURSOR = () => {
  ...
  if (document.body) inst();
  else document.addEventListener("DOMContentLoaded", inst);
};
    ...
    .evaluate((text) => {
      ...
      if (!el) {
        ...
        Object.assign(el.style, {
          ...
          color: "#fff",
          background: "linear-gradient(180deg,rgba(20,20,30,.94),rgba(20,20,30,.72))",
          padding: "11px 20px",
          textAlign: "center",
        });
...
const check = async (label) => {
  ...
  for (let i = 0; i < 24; i++) {
    try {
      ft = await appFrame(page).locator("body").innerText({ timeout: 2500 });
    } catch {
      ft = "";
    }
    if (/Welcome back|Monthly Usage|Choose Your Plan|Brand voice|Get started|Shop domain|>Log in</i.test(ft))
      break;
...
const clickNav = async (name) => {
  const l = appFrame(page)
    .getByRole("link", { name: new RegExp("^" + name + "$", "i") })
    .first()
    .or(appFrame(page).locator(`s-link:has-text("${name}")`).first());
  try {
...
try {
  ...
  await caption("Fresh dev store, incognito (3rd-party cookies blocked). App dashboard loads.");
  if (await check("1-dashboard")) ok = false;
  await caption("Open Plans & Billing");
  if (!(await clickNav("Plans & Billing"))) await gotoTop(`${APP_ROOT}/app/plans`);
  if (await check("2-plans")) ok = false;
  await caption("Click Dashboard (the reviewer's move that used to dead-end)");
  if (!(await clickNav("Dashboard"))) await gotoTop(`${APP_ROOT}/app`);
  if (await check("3-dashboard-via-nav")) ok = false;
  await caption("Reload /app — the reviewer's kill move");
  await page.reload({ waitUntil: "domcontentloaded" }).catch(() => {});
  await page.waitForTimeout(3500);
  if (await check("4-reload-app")) ok = false;
  await caption("Settings, then back to Dashboard");
  if (!(await clickNav("Settings"))) await gotoTop(`${APP_ROOT}/app/settings`);
  await check("5-settings");
  if (!(await clickNav("Dashboard"))) await gotoTop(`${APP_ROOT}/app`);
  if (await check("6-back-to-dashboard")) ok = false;
  await caption("Reload a sub-page");
  await gotoTop(`${APP_ROOT}/app/plans`);
  await page.reload({ waitUntil: "domcontentloaded" }).catch(() => {});
  await page.waitForTimeout(3500);
  if (await check("7-reload-plans")) ok = false;
  await caption("Click the app name / home");
  await gotoTop(`${APP_ROOT}`);
  if (await check("8-app-name-home")) ok = false;
