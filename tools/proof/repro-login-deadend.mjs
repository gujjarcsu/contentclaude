...
const B64 = Buffer.from("contentpilot-dev2.myshopify.com/admin")
  .toString("base64")
  .replace(/\+/g, "-")
  .replace(/\//g, "_")
  .replace(/=+$/, "");
const cases = [
  {
    name: "A) / with host+embedded, NO shop (admin home-nav shape)",
    url: `https://app.navaal.ai/?host=${B64}&embedded=1`,
  },
  {
    name: "B) / with shop present",
    url: `https://app.navaal.ai/?shop=contentpilot-dev2.myshopify.com&host=${B64}&embedded=1`,
  },
  { name: "C) / with NO params at all", url: `https://app.navaal.ai/` },
  {
    name: "D) /auth/login with host (should auto-recover, no form)",
    url: `https://app.navaal.ai/auth/login?host=${B64}&embedded=1`,
  },
];
