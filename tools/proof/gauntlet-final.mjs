...
const check = async (label) => {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    if (/Welcome back|Monthly Usage|Choose Your Plan|Brand voice|Shop domain|>Log in<|name="shop"/i.test(ft))
      break;
...
try {
  ...
  await caption(
    "Incognito condition (3rd-party cookies blocked). App open — the shop cookie is now persisted.",
  );
  await check("1-open");
  ...
  await caption(
    "Now FORCE the exact dead-end: a bare /app load with NO shop/host → must recover, no form, no 404",
  );
