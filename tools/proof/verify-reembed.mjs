...
async function trace(label, url, headers) {
  ...
  const openTarget =
    (body.match(/adminDest\s*=\s*"([^"]*)"/) || [])[1] ||
    (body.match(/https?:\/\/[^"'\s]+/) || [])[0] ||
    "(client-derived from App Bridge)";
  const framable = /frame-ancestors[^;]*admin\.shopify\.com/i.test(csp);
  console.log(`\n### ${label}`);
  console.log(`   chain: ${chain.join("  |  ")}`);
  console.log(`   final status: ${status}`);
  console.log(
    `   FORM: ${form ? "YES" : "no"}   RE-EMBED(App Bridge): ${reembed ? "YES" : "no"}   admin-framable: ${framable ? "yes" : "NO"}`,
  );
  if (reembed) console.log(`   → window.open: ${openTarget}`);
  return { form, reembed, status, framable };
}

const B64 = Buffer.from("q491r2-si.myshopify.com/admin")
  .toString("base64")
  .replace(/\+/g, "-")
  .replace(/\//g, "_")
  .replace(/=+$/, "");
const R = {};
R.a = await trace(
  "A) /auth/login  iframe, NO params (validateShopAndHostParams dead-end)",
  "https://app.navaal.ai/auth/login",
  iframeHeaders,
);
R.b = await trace(
  "B) /auth/login  TOP-LEVEL document, no params (genuine external)",
  "https://app.navaal.ai/auth/login",
  topHeaders,
);
R.c = await trace("C) /  iframe, NO params", "https://app.navaal.ai/", iframeHeaders);
R.d = await trace(
  "D) /auth/login  iframe WITH host",
  `https://app.navaal.ai/auth/login?host=${B64}&embedded=1`,
  iframeHeaders,
);
R.e = await trace(
  "E) /app  iframe, no session/params -> should end at re-embed (via /auth/login)",
  "https://app.navaal.ai/app",
  iframeHeaders,
);
...
const pass =
  R.a.reembed &&
  !R.a.form &&
  R.a.framable && // iframe dead-end recovers + embeddable
  R.b.form && // external still gets the form
  R.c.reembed &&
  R.c.framable && // root iframe recovers + embeddable
