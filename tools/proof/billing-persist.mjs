...
try {
  ...
  if (/verify you are human|needs to be verified/i.test(await page.content().catch(() => "")))
  ...
  if (vids.length) {
    fs.renameSync(`${OUT}/video/${vids[0]}`, `${OUT}/billing-persist.webm`);
    log(`video: ${OUT}/billing-persist.webm`);
  }
