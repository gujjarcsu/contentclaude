...
const child = spawn(
  ...
  [
    ...
    `${APP_ROOT}/app/products`,
  ],
  { detached: true, stdio: "ignore" },
);
child.unref();
const waitCDP = async (d) => {
  while (Date.now() < d) {
    try {
...
if (!(await waitCDP(Date.now() + 40000))) {
  console.error("no debug endpoint");
  process.exit(3);
}
...
while (Date.now() < deadline) {
  const p = context
    .pages()
    .find((x) => new RegExp(`admin\\.shopify\\.com/store/${STORE}/apps/${APP}`).test(x.url()));
  if (p) {
    const hasFrame = await p
      .locator('iframe[src*="navaal"]')
      .count()
      .catch(() => 0);
    if (hasFrame) {
      page = p;
...
if (!page) {
  console.error("TIMEOUT: app never reached.");
  process.exit(1);
}
...
const clickTitle = async () => {
  for (const c of [
    ...
    page
      .locator('a[href$="/apps/navaal-seo-geo-content"], a[href*="/apps/navaal-seo-geo-content?"]')
...
try {
  const capP = capture();
  await caption("Fresh dev store · incognito · on the Products page");
  ...
  log("title clicked: " + (await clickTitle()));
  await page.waitForTimeout(6500);
  await caption("→ Dashboard renders — no login form");
  if (await check("2-after-title-click")) ok = false;
  await page.waitForTimeout(2000);
  for (const nm of ["Products", "Optimise Store", "Plans & Billing", "SEO Audit", "Settings", "Dashboard"]) {
    await caption(`Nav → ${nm}`);
    await clickNav(nm);
    await page.waitForTimeout(2000);
    if (await check(`nav-${nm}`)) ok = false;
  }
  await caption("Plans page — click the back arrow");
  await clickNav("Plans & Billing");
  await page.waitForTimeout(2000);
  const back = appFrame().locator('button[aria-label*="Back" i], a[aria-label*="Back" i]').first();
  try {
  ...
  await page.waitForTimeout(2500);
  if (await check("back-arrow")) ok = false;
  await caption("Browser Back button");
  await page.goBack({ waitUntil: "domcontentloaded" }).catch(() => {});
  await page.waitForTimeout(2500);
  if (await check("browser-back")) ok = false;
  await caption("App title, every nav item, back arrow, browser back — the login form never appears ✓");
