...
let page =
  ctx.pages().find((p) => /admin\.shopify\.com\/store/.test(p.url())) ||
  ctx.pages()[0] ||
  (await ctx.newPage());
...
const injectOverlays = async () => {
  ...
  for (const f of page.frames()) {
    if (/navaal|app\.navaal\.ai/.test(f.url())) {
      await f.evaluate(CURSOR).catch(() => {});
    }
  }
};
    ...
    .evaluate((text) => {
      ...
      if (!el) {
        ...
        Object.assign(el.style, {
          ...
          zIndex: 2147483646,
          pointerEvents: "none",
          font: "700 18px/1.4 -apple-system,Segoe UI,Roboto,sans-serif",
          color: "#fff",
...
const clickNav = async (name) => {
  const l = appFrame()
    ...
    .first()
    .or(appFrame().locator(`s-link:has-text("${name}")`).first());
  try {
    await l.hover({ timeout: 4000 });
...
const driver = async () => {
  ...
  await caption("Live production build — git SHA on screen");
  await page.waitForTimeout(2600);
  await gotoTop(`${APP_ROOT}/app`);
  await caption("Fresh dev store · genuine incognito · Navaal dashboard");
  ...
  await appFrame()
    ...
    .catch(() => {});
  await injectOverlays();
  await caption("Reload /app → Dashboard loads, NO login form");
  await page.waitForTimeout(3200);
  await caption("Open Settings");
  if (!(await clickNav("Settings"))) await gotoTop(`${APP_ROOT}/app/settings`);
  await injectOverlays();
  await page.waitForTimeout(2400);
  await caption("Back to Dashboard");
  if (!(await clickNav("Dashboard"))) await gotoTop(`${APP_ROOT}/app`);
  await injectOverlays();
  await page.waitForTimeout(2600);
  ...
  await injectOverlays();
  await page.waitForTimeout(2600);
  await caption("Every navigation loads the app — the login form never appears ✓");
  await page.waitForTimeout(3200);
};
...
const r = spawnSync(
  ...
  [
    ...
    "-movflags",
    "+faststart",
    `${OUT}/reviewer-proof.mp4`,
  ],
  { stdio: "inherit" },
);
console.log("ffmpeg exit:", r.status);
console.log(
  fs.existsSync(`${OUT}/reviewer-proof.mp4`)
    ? `VIDEO: ${OUT}/reviewer-proof.mp4 (${(fs.statSync(`${OUT}/reviewer-proof.mp4`).size / 1e6).toFixed(1)} MB)`
    : "NO VIDEO",
);
process.exit(0);
