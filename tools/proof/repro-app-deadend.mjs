...
const cases = [
  {
    name: "A) /app  host+embedded, NO id_token, NO cookie (reviewer reload)",
    url: `https://app.navaal.ai/app?host=${B64}&embedded=1`,
  },
  { name: "B) /app  bare, no params, no cookie", url: `https://app.navaal.ai/app` },
  {
    name: "C) /app/plans host+embedded, no cookie",
    url: `https://app.navaal.ai/app/plans?host=${B64}&embedded=1`,
  },
  {
    name: "D) /app/settings host+embedded, no cookie",
    url: `https://app.navaal.ai/app/settings?host=${B64}&embedded=1`,
  },
];
...
for (const c of cases) {
  ...
  page.on("response", (r) => {
    const s = r.status();
    if (s >= 300 && s < 400)
      chain.push(`${s} ${new URL(r.url()).pathname} -> ${r.headers()["location"] || "?"}`);
  });
  let finalUrl = "",
    hasForm = false,
    title = "",
