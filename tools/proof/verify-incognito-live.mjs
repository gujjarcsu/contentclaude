...
while (Date.now() < deadline) {
  const p = context
    ...
    .find(
      (x) =>
        /admin\.shopify\.com\/store\//.test(x.url()) &&
        !/\/login|accounts\.shopify\.com|\/oauth\//.test(x.url()),
    );
  if (p) {
    page = p;
    break;
...
if (!page) {
  console.error("TIMEOUT: admin never reached.");
  process.exit(1);
}
console.log("Admin detected — starting the verification flow.\n");

const appFrame = (pg) =>
  pg.frameLocator('iframe[name^="app-iframe"], iframe[src*="navaal"], iframe[src*="app.navaal.ai"]').first();
const results = [];
const check = async (label) => {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    if (
      /Welcome back|Monthly Usage|Choose Your Plan|Brand voice|Shop domain|>Log in<|name="shop"/i.test(
        frameText,
      )
    )
      break;
  ...
  const ok =
    /Welcome back|Monthly Usage|Choose Your Plan|Brand voice|Professional|Free/i.test(frameText) && !form;
  results.push({ label, url: page.url(), form, ok, len: frameText.trim().length });
  console.log(
    `${form ? "❌ FORM" : ok ? "✅ ok" : "· "} — ${label} | form=${form} len=${frameText.trim().length} | ${page.url()}`,
  );
  await page
    .screenshot({
...
try {
  ...
  await page.waitForTimeout(3000);
  await check("2-dashboard-via-nav");
  // The reviewer's kill move: reload /app.
  await gotoTop(`${APP_ROOT}/app`);
  await page.reload({ waitUntil: "domcontentloaded" }).catch(() => {});
  await page.waitForTimeout(3000);
  await check("3-reload-app");
  (await clickNav("Settings")) || (await gotoTop(`${APP_ROOT}/app/settings`));
  await check("4-settings");
  (await clickNav("Dashboard")) || (await gotoTop(`${APP_ROOT}/app`));
  await check("5-back-to-dashboard");
  // reload a sub-page too
  await gotoTop(`${APP_ROOT}/app/plans`);
  await page.reload({ waitUntil: "domcontentloaded" }).catch(() => {});
  await page.waitForTimeout(3000);
  await check("6-reload-plans");
} catch (e) {
