...
const check = async (label, { coldPath = false } = {}) => {
  ...
  for (let i = 0; i < 28; i++) {
    ...
    if (
      /Welcome back|Monthly Usage|Total Products|Choose Your Plan|Brand voice|Shop domain|>Log in<|name="shop"/i.test(
        frameText,
      )
    )
      break;
  ...
  const dash = /Welcome back|Monthly Usage|Total Products|Choose Your Plan|Plans & Billing|Brand voice/i.test(
    frameText,
  );
  const rec = {
    label,
    ...
    form,
    recovered: dash && !form,
    len: frameText.trim().length,
  };
  results.push(rec);
  log(
    `${form ? "❌ FORM" : rec.recovered ? "✅ recovered" : "· ok"} — ${label} | form=${form} len=${rec.len}`,
  );
  await page
    .screenshot({
      path: `${OUT}/${String(results.length).padStart(2, "0")}-${label.replace(/[^a-z0-9]+/gi, "_").slice(0, 34)}.png`,
    })
    .catch(() => {});
  return rec;
};
...
try {
  ...
  for (const [label, path] of [
    ...
    await caption(
      `Forcing a bare "${path}" load with no shop/host (the exact dead-end) → must recover, no login form`,
    );
    const fr = frameOf(page);
    if (fr) {
      await fr.evaluate((p) => window.location.assign(p), path).catch(() => {});
    } else {
