...
const { db, log, cache } = vi.hoisted(() => {
  ...
  return {
    db: {
      ...
      upgradePrompt: { upsert: fn(), findFirst: fn(), update: fn(), updateMany: fn(), count: fn(), findMany: fn() },
...
vi.mock("../../app/utils/cache.server.js", () => ({ getCache: cache.getCache, invalidateCache: cache.invalidateCache, setCache: vi.fn() }));
...
describe("catalogGaps", () => {
  const node = (i, desc, extra = {}) => ({ id: `gid://shopify/Product/${i}`, title: `P${i}`, description: desc, seo: { title: "t", description: "d" }, featuredMedia: { preview: { image: { url: `u${i}` } } }, ...extra });
  const resp = (edges, hasNextPage, endCursor = "c") => ({ json: async () => ({ data: { products: { edges: edges.map((n) => ({ node: n })), pageInfo: { hasNextPage, endCursor } } } }) });
  ...
  it("returns missing-then-thin candidates (image first), excludes products Navaal already touched, reports truncation", async () => {
    const admin = { graphql: vi.fn(async () => resp([
    ...
    expect(r.candidates[0]).toMatchObject({ numericId: "4", tier: 0, mode: "generate", imageUrl: "u4", descriptionLength: 0 });
  ...
  it("pages up to maxPages until enough candidates, and an Admin error returns { error: true } — never throws, never a count", async () => {
    let call = 0;
    const admin = { graphql: vi.fn(async () => { call += 1; return call === 1 ? resp([node(1, "x".repeat(60))], true, "c1") : resp([node(2, ""), node(3, "")], false); }) };
    ...
    const bad = { graphql: vi.fn(async () => { throw new Error("boom"); }) };
...
describe("reviewAsk — pure rules", () => {
  it("holdFor per Shopify code", () => {
    ...
    expect(review.holdFor("already-reviewed", { now })).toEqual({ terminal: true, shown: false, nextEligibleAt: null });
    expect(review.holdFor("merchant-ineligible", { now }).terminal).toBe(true);
    expect(review.holdFor("annual-limit-reached", { now }).nextEligibleAt.toISOString()).toBe("2027-09-09T00:00:00.000Z");
    expect(review.holdFor("cooldown-period", { now }).nextEligibleAt.toISOString()).toBe("2026-11-08T00:00:00.000Z");
    ...
    expect(review.holdFor("recently-installed", { now, installAt: null }).nextEligibleAt.toISOString()).toBe("2026-09-09T01:00:00.000Z"); // now + 1 h
    ...
    expect(review.holdFor("unavailable", { now }).nextEligibleAt.toISOString()).toBe("2026-09-16T00:00:00.000Z");
    expect(review.holdFor("some-new-code", { now }).nextEligibleAt.toISOString()).toBe("2026-11-08T00:00:00.000Z");
  });
  ...
  it("decideReviewAsk ladder", () => {
    ...
    expect(review.decideReviewAsk({ ...base, reviewLastAskedAt: DAYS(1) }, { reviewRequestedAt: DAYS(10) }).eligible).toBe(true); // legacy only when never asked under the new code
...
describe("reviewAsk — openReviewAsk / recordReviewOutcome", () => {
  const ROW = { shop: SHOP, installedAt: DAYS(3), reviewAskCount: 0, reviewDoneAt: null, reviewNextEligibleAt: null, reviewLastAskedAt: null };
  ...
  it("opens exactly one attempt per eligible moment via the optimistic claim, with a 60-day pending hold", async () => {
    ...
    const [a, b] = await Promise.all([
      review.openReviewAsk({ shop: SHOP, surface: "review_page", trigger: "first_publish", publishedCount: 3 }),
      review.openReviewAsk({ shop: SHOP, surface: "review_page", trigger: "first_publish", publishedCount: 3 }),
    ]);
    ...
    expect(db.reviewRequestAttempt.create.mock.calls[0][0].data).toMatchObject({ shop: SHOP, surface: "review_page", trigger: "first_publish", publishedCount: 3, attemptNo: 1 });
    expect(db.reviewRequestAttempt.create.mock.calls[0][0].data.installAgeSeconds).toBeGreaterThan(3 * 86_000);
  });
  ...
  it("recordReviewOutcome writes the code once in a transaction and sets the real hold (success → done + shown)", async () => {
    db.reviewRequestAttempt.findUnique.mockResolvedValueOnce({ id: "att_1", shop: SHOP, surface: "review_page", trigger: "first_publish", code: null });
    db.reviewRequestAttempt.update.mockResolvedValue({});
    db.shop.findUnique.mockResolvedValue(ROW);
    const r = await review.recordReviewOutcome(SHOP, "att_1", { code: "success", success: true, message: "Review modal displayed" });
    expect(r).toMatchObject({ status: 200, terminal: true, shown: true });
    expect(db.$transaction).toHaveBeenCalledTimes(1);
    expect(db.reviewRequestAttempt.update.mock.calls[0][0].data).toMatchObject({ code: "success", success: true, message: "Review modal displayed", nextEligibleAt: null });
    ...
    expect((await review.recordReviewOutcome("other.myshopify.com", "att_1", { code: "success" })).status).toBe(404);
  });

  it("recordReviewOutcome: cooldown-period → +60 d not terminal; cancelled → +24 h not terminal", async () => {
    for (const [code, minDays, maxDays] of [["cooldown-period", 59, 61], ["cancelled", 0.9, 1.1]]) {
...
describe("planFit", () => {
  it("fitPlanFor matrix", () => {
    expect(planFit.fitPlanFor({ n: 3, currentPlan: "free" })).toMatchObject({ planName: "starter", covers: true, monthsToCover: 1, priceLabel: "$9.99/mo" });
    ...
    expect(planFit.fitPlanFor({ n: 1500, currentPlan: "free" })).toMatchObject({ planName: "pro", covers: false, monthsToCover: 2 });
  ...
  it("quotaResetDate crosses the year boundary in UTC; fmtDay/fmtMonth", () => {
    expect(planFit.quotaResetDate(new Date("2026-12-15T10:00:00Z")).toISOString()).toBe("2027-01-01T00:00:00.000Z");
  ...
  it("quotaGapTitle is the exact line from the brief", () => {
    ...
    expect(planFit.quotaGapTitle({ n: 12, fit: starter })).toBe("12 products still need content · Starter covers 50/month");
    expect(planFit.quotaGapTitle({ n: 1, fit: starter })).toBe("1 product still needs content · Starter covers 50/month");
    expect(planFit.quotaGapTitle({ n: 300, truncated: true, fit: planFit.fitPlanFor({ n: 300, currentPlan: "free" }) })).toBe("At least 300 products still need content · Professional covers 1000/month");
  });
});
...
describe("upgradePrompts", () => {
  ...
  it("getUpsell: null while quota remains or plan not active; neutral when N = 0 or no fit; condition row upserted (not a show)", async () => {
    expect(await prompts.getUpsell({ shop: SHOP, plan: PLAN, usageCount: 10, surface: "dashboard", n: 5 })).toBeNull();
    expect(await prompts.getUpsell({ shop: SHOP, plan: { ...PLAN, status: "frozen" }, usageCount: 25, surface: "dashboard", n: 5 })).toBeNull();
    const neutral = await prompts.getUpsell({ shop: SHOP, plan: PLAN, usageCount: 25, surface: "dashboard", n: 0 });
    expect(neutral).toMatchObject({ neutral: true, fit: null, promptId: null, resetDate: expect.any(String), monthName: expect.any(String) });
    expect(db.upgradePrompt.upsert).not.toHaveBeenCalled();
    db.upgradePrompt.upsert.mockResolvedValue({ id: "up_1" });
    const u = await prompts.getUpsell({ shop: SHOP, plan: PLAN, usageCount: 25, surface: "dashboard", n: 12, truncated: true, nDefinition: "catalog_gaps" });
    expect(u).toMatchObject({ neutral: false, n: 12, truncated: true, promptId: "up_1", fit: { planName: "starter" } });
    const call = db.upgradePrompt.upsert.mock.calls[0][0];
    expect(call.where.shop_trigger_surface_month).toMatchObject({ shop: SHOP, trigger: "quota_exhausted", surface: "dashboard" });
    expect(call.create).toMatchObject({ n: 12, truncated: true, fitPlanName: "starter", currentPlan: "free", monthlyLimit: 25 });
  ...
  it("getUpsell scans the catalog when N is not given and stays silent on a scan error", async () => {
    ...
    expect(await prompts.getUpsell({ admin: { graphql: vi.fn() }, shop: SHOP, plan: PLAN, usageCount: 25, surface: "products" })).toBeNull();
    ...
    const u = await prompts.getUpsell({ admin: { graphql: vi.fn() }, shop: SHOP, plan: PLAN, usageCount: 25, surface: "products" });
  ...
  it("markPromptEvent is shop-scoped, counts shows, sets firstShownAt once", async () => {
    ...
    expect(db.upgradePrompt.updateMany.mock.calls[0][0].where).toEqual({ id: "up_1", shop: SHOP, firstShownAt: null });
  ...
  it("markPromptArrived (first time only) and markSubscribeRequested", async () => {
    ...
    expect(db.upgradePrompt.updateMany.mock.calls[0][0].where).toEqual({ id: "up_1", shop: SHOP, arrivedAtPlansAt: null });
    await prompts.markSubscribeRequested(SHOP, "up_1", "Starter Plan");
    expect(db.upgradePrompt.updateMany.mock.calls[1][0].data).toMatchObject({ planKeyRequested: "Starter Plan" });
  });

  it("attributePlanChoice: only on a NEW subscription id, once per charge, only to a row the merchant acted on", async () => {
    expect(await prompts.attributePlanChoice(SHOP, { planName: "starter", chargeId: "gid://c/1", prevChargeId: "gid://c/1" })).toBeNull();
    expect(db.upgradePrompt.count).not.toHaveBeenCalled();
    db.upgradePrompt.count.mockResolvedValue(1);
    expect(await prompts.attributePlanChoice(SHOP, { planName: "starter", chargeId: "gid://c/2", prevChargeId: null })).toBeNull();
    db.upgradePrompt.count.mockResolvedValue(0);
    db.upgradePrompt.findFirst.mockResolvedValue(null);
    expect(await prompts.attributePlanChoice(SHOP, { planName: "starter", chargeId: "gid://c/2", prevChargeId: null })).toBeNull();
    expect(log.info).toHaveBeenCalledWith(expect.objectContaining({ event: "upgrade_organic" }), expect.any(String));
    ...
    db.upgradePrompt.findFirst.mockResolvedValue({ id: "up_9", trigger: "quota_exhausted", surface: "dashboard", fitPlanName: "starter" });
    db.upgradePrompt.update.mockResolvedValue({});
    expect(await prompts.attributePlanChoice(SHOP, { planName: "starter", chargeId: "gid://c/2", prevChargeId: null, prevPlanName: "free" })).toBe("up_9");
    expect(db.upgradePrompt.update.mock.calls[0][0].data).toMatchObject({ planChosen: "starter", chargeId: "gid://c/2", planChosenFrom: "free" });
    db.upgradePrompt.count.mockRejectedValue(new Error("down"));
    expect(await prompts.attributePlanChoice(SHOP, { planName: "starter", chargeId: "gid://c/3" })).toBeNull();
  });
...
describe("ttvReport", () => {
  const rows = [
    { shop: "a", installSource: "unknown", installedAt: DAYS(1), firstDraftSeenAt: new Date(DAYS(1).getTime() + 90_000), firstDraftSource: "quick_start", firstPublishAt: new Date(DAYS(1).getTime() + 600_000), firstPublishSource: "review_page" },
    { shop: "b", installSource: "app_store:search", installedAt: DAYS(2), firstDraftSeenAt: new Date(DAYS(2).getTime() + 30_000), firstDraftSource: "quick_start" },
    { shop: "c", installSource: "unknown", installedAt: DAYS(3), uninstalledAt: DAYS(2.5), productCountAtFirstLoad: 0 },
    { shop: "e", installSource: "unknown", installedAt: DAYS(40), reinstalledAt: DAYS(4), firstDraftSeenAt: DAYS(39), redactedAt: DAYS(1) },
  ];

  it("summarizeGrowth: medians over reached installs, censored median, inconsistent flagged, redacted retained", () => {
    const r = ttv.summarizeGrowth({ cohortRows: rows, attempts: [{ shop: "a", code: "success" }, { shop: "b", code: "recently-installed" }, { shop: "b", code: null }], prompts: [{ trigger: "quota_exhausted", shownCount: 2, ctaClickedAt: new Date(), planChosen: "starter" }, { trigger: "quota_exhausted", shownCount: 0 }], excludedShops: ["x"], cohortRequested: 20 });
    expect(r.cohortSize).toBe(4);
    expect(r.smallSample).toBe(true);
    expect(r.draft).toMatchObject({ n: 4, reached: 2, notReached: 2, uninstalledWithoutReaching: 1, zeroProducts: 1, medianSeconds: 60, under120s: 2, bySource: { quick_start: 2 }, censoredMedian: "not reached" });
    ...
    expect(r.reviewAsk).toEqual({ attempts: 3, attemptsByCode: { success: 1, "recently-installed": 1, pending: 1 }, shopsAsked: 2, shopsShown: 1 });
    expect(r.upgradePrompts.byTrigger.quota_exhausted).toMatchObject({ rows: 2, shown: 1, ctaClicked: 1, planChosen: 1 });
  ...
  it("reached === 0 → null median with a note; censored median numeric when more than half reached", () => {
    const none = ttv.summarizeGrowth({ cohortRows: [{ shop: "z", installSource: "unknown", installedAt: DAYS(1) }] });
